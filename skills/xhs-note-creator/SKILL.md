...
- 使用简短的句子和段落
- **换行处理**：每段之间用空行分隔，确保渲染后格式清晰
- **Tags 格式**：在文案末尾单独一行，用空格分隔，**不要加 `#` 前缀**
  - ✅ 正确：`OpenClaw AI 助理 自动化 效率工具`
  - ❌ 错误：`#OpenClaw #AI 助理 #自动化`
...
**推荐方式：从 Markdown 文件发布（自动提取正文和 tags）**

```bash
python scripts/publish_xhs.py --title "笔记标题" --md content.md --images cover.png card_1.png card_2.png
```
...
**参数说明：**
- `--title/-t`: 笔记标题（不超过 20 字）
- `--md`: Markdown 文件路径（推荐，自动提取正文和 tags）
- `--desc/-d`: 手动指定笔记描述
- `--images/-i`: 图片文件列表（封面 + 正文卡片）
- `--api-mode`: 使用 API 模式发布（需要 xhs-api 服务）
- `--private`: 设为私密笔记
- `--post-time`: 定时发布（格式："2024-12-01 10:00:00"）
